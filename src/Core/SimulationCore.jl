...
@inline function runsimulation(
    ...
    for iter = 1:datanum
        ...
        # disturbance input
        attidistinput = transpose(C_ECI2Body) * calc_attitudedisturbance(distconfig, distinternals, attitudemodel.inertia, time[iter], orbitdata.angularvelocity[iter], C_ECI2Body, C_ECI2RAT, orbitdata.LVLH[iter].z, Ts)
        # control input
